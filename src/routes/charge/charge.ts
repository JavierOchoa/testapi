...
app.get('/one', async (_req, res) => {
    albumApi.map(async (element: any, i: number) => {
        if(i<2500){
    ...
    songApiAux.map(async (element: any, i: number) => {
        if(i<2500){
    ...
    songGenreApi.map(async (song: any, i: number) => {
        if(i<2500){
    ...
    genreApi.map(async (element: any, i: number) => {
        if(i<2500){
...
app.get('/two', async (_req, res) => {
    artistApi.map(async (element: any, i: number) => {
        if(i<2500){
    ...
    artist_Song.map(async (element: any, i: number) => {
        if(i<2500){
    ...
    // song album
    song_Album.map(async (element: any, i: number) => {
        if(i<2500){
        const songDb = await song.findOne({ where: { dz_Id: element.idSong } })
        const albumDb = await album.findOne({ where: { dz_Id: element.idAlbum } })
        albumDb.addSong(songDb)
    ...
    artistSongDb.map(async (element: any, i: number) => {
        if(i<2500){
        const songDb = await song.findOne({ where: { dz_Id: element.idSong } })
    ...
    albumGenreDb.map(async (element: any, i: number) => {
        if(i<2500){
        const albumDb = await album.findOne({ where: { dz_Id: element.idAlbum } })
        const genreDb = await genre.findOne({ where: { dz_Id: element.idGenre } })
    ...
    artistAlbumDb.map(async (element: any, i: number) => {
        if(i<2500){
        const artistDb = await artist.findOne({ where: { dz_Id: element.idArtist } })
        const albumDb = await album.findOne({ where: { dz_Id: element.idAlbum } })
